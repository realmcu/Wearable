...
#if F_APP_SPP_CAPTURE_DSP_DATA_2
    //if ((app_cfg_const.enable_dsp_capture_data_by_spp) &&          todo   temp code
...
void app_sniff_mode_b2s_disable_all(uint16_t flag)
{
    ...
    if (app_cfg_nv.bud_role != REMOTE_SESSION_ROLE_SECONDARY)
    {
        ...
        for (app_idx = 0; app_idx < MAX_BR_LINK_NUM; app_idx++)
        {
            if (br_db.br_link[app_idx].used)
            {
                bt_sniff_mode_disable(br_db.br_link[app_idx].bd_addr);
            }
        }
    }
}
    ...
    if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
    {
        if (app_bt_policy_get_b2b_connected())
        {
            for (app_idx = 0; app_idx < MAX_BR_LINK_NUM; app_idx++)
            {
                if (app_check_b2b_link_by_id(app_idx))
                {
                    if (0 != br_db.br_link[app_idx].sniff_mode_disable_flag)
                    {
...
void app_sniff_mode_roleswap_suc(void)
{
    ...
    if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
    {
        for (app_idx = 0; app_idx < MAX_BR_LINK_NUM; app_idx++)
        {
            if (app_check_b2b_link_by_id(app_idx))
            {
                br_db.br_link[app_idx].sniff_mode_disable_flag = g_b2b_link_disable_flag_temp;
            ...
            {
                br_db.br_link[app_idx].sniff_mode_disable_flag = g_b2s_link_disable_flag_temp;
