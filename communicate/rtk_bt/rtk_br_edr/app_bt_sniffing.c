...
static uint8_t timer_idx_retry_sniffing = 0;
...
typedef enum
{
    ...
    APP_TIMER_RETRY_SNIFFING         = 0x04,
    APP_TIMER_SNIFFING_STATE_PROTECT = 0x05,
    APP_TIMER_CHECK_ROLE_SWITCH      = 0x06,
    APP_TIMER_RECONN_INACTIVE_LINK   = 0x07,
...
static T_SNIFFING_CHECK_RESULT app_bt_sniffing_check(uint8_t *bd_addr)
{
    ...
    APP_PRINT_TRACE1("app_bt_sniffing_check: check_result %d", check_result);
    return check_result;
}
...
{
    APP_PRINT_TRACE2("app_bt_sniffing_handle_acl_link_disconn:  bud_role %d, is_b2b_link %d",
    ...
    case APP_BT_SNIFFING_EVENT_A2DP_SNIFFING_STOPPED:
        if (p_link->sniffing_type == BT_SNIFFING_TYPE_A2DP)
        {
            if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
            {
                ...
                bt_sniffing_link_disconnect(bd_addr);
#if F_APP_DUAL_AUDIO_TWS_SPATIAL_AUDIO
                if (app_cfg_const.mems_support && app_cfg_nv.dual_audio_space == 1)
                {
                    app_sensor_mems_stop(NULL);
                }
#endif
            }
        }
    ...
    case BT_EVENT_ACL_SNIFFING_DISCONN_CMPL:
        {
            ...
            if (p_link != NULL)
            {
                ...
                {
                    // retry sniffing
                    app_start_timer(&timer_idx_retry_sniffing, "retry_sniffing",
                                    bt_sniffing_timer_id, APP_TIMER_RETRY_SNIFFING, p_link->id, false,
