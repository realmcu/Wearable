...
static void rtc_clock_sys_update(void)
{
    ...
    IO_PRINT_INFO6("year:%d,month:%d,day:%d,hour:%d,minute:%d,second:%d\r\n",
                   RtcClockSys.global_time.year,
                   RtcClockSys.global_time.month, RtcClockSys.global_time.day,
                   RtcClockSys.global_time.hour, RtcClockSys.global_time.minutes,
                   RtcClockSys.global_time.seconds);
...
void RTC_Handler()
{
    ...
    IO_PRINT_INFO3("RTC->INT_SR = 0x%x, RTC->CR0 = 0x%x, RTC INT_MASK = 0x%x", RTC->INT_SR, RTC->CR0,
                   RTC->INT_MASK);
    if (RTC_GetINTStatus(RTC_INT_CMP0) == SET)
    {
        IO_PRINT_INFO0("RTC_INT_CMP0");
...
void rtc_clock_start(void)
{
    ...
    IO_PRINT_INFO1("rtc_clock clock_start second_diff_value: %d",
