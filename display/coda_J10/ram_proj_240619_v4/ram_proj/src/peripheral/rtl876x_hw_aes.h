...
#define HW_AES_SET_IRK(pIRK, cnt)           for (uint8_t i = 0;i < cnt;i ++) {\
                                                uint32_t *ptr = (uint32_t *)pIRK;\
                                                HWAES->IRK[(cnt - 1) - i] = ptr[i];\
                                            }
#define HW_AES_GET_IRK(pIRK, cnt)           for (uint8_t i = 0;i < cnt;i ++) {\
                                                uint32_t *ptr = (uint32_t *)pIRK;\
                                                ptr[i] = HWAES->IRK[(cnt - 1) - i];\
                                            }
#define HW_AES_SET_IV(pIV)                  for (uint8_t i = 0;i < 4;i ++) {\
                                                uint32_t *ptr = (uint32_t *)pIV;\
                                                HWAES->iv[3 - i] = ptr[i];\
                                            }
#define HW_AES_GET_IV(pIV)                  for (uint8_t i = 0;i < 4;i ++) {\
                                                uint32_t *ptr = (uint32_t *)pIV;\
                                                ptr[i] = HWAES->iv[3 - i];\
                                            }
...
#define AES_LOG(...)
    ...
    extern void (*hw_aes_setkey)(const uint32_t *aesKey, uint32_t key_bits, uint8_t key_sel);
    extern bool (*hw_aes_cpu_operate)(uint32_t *in, uint32_t *out, uint32_t word_len, bool isEncrypt);

    void hw_aes_init(HW_AES_WORK_MODE aes_mode, uint32_t *iv);

#if AES_SUPPORT_DMA_MODE == 1
    extern void (*hw_aes_dma_channel_init)(AES_DMA_CFG *rx_cfg, AES_DMA_CFG *tx_cfg,
                                           volatile bool *semaphore);
    extern bool (*hw_aes_dma_operate)(uint32_t *in, uint32_t *out, uint32_t word_len, bool isEncrypt);
    ...
    bool hw_cmac(AES_CMAC_CFG *cmac_cfg, uint32_t *out);
