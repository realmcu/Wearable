...
bool hw_aes_init(const uint32_t *aesKey, uint32_t *iv, HW_AES_WORK_MODE work_mode, bool isAes256)
{
    hw_aes_set_clk(true);
                        ...
                        uint8_t dma_rx_ch_num, uint8_t dma_tx_ch_num, bool isMac)
{
    ...
    /* enable DMA clock for AES DMA mode */
...
void GDMA0_Channel6_Handler(void)
{
...
void GDMA0_Channel7_Handler(void)
{
                           ...
                           const uint32_t *p_aes_key, int mode)
{
    ...
    bool result = hw_aes_init((uint32_t *)p_aes_key, p_iv, AES_CBC, false);
                             ...
                             char *K2)
{
    ...
    hw_aes_init((uint32_t *)key, NULL, AES_ECB, false);
...
uint32_t get_secure_reg_cfg_val(uint32_t in_val)
{
    ...
    reg[0] = in_val;
    hw_aes_init(secure_key[HWAES->SEC_STS.irk_cnt], NULL, AES_ECB, false);
...
extern void share_ram_crontrol(uint8_t mode);
void share_dsp_and_cache_ram_rom(void)
{
    /* Share DSP RAM: secure register 0x68[0], share Cache RAM: secure register 0x68[1],
       should encode the whole 32 bits and fill to HWAES 0x68 32 bits at the same time */
    uint32_t hwaes_val = 0;
    hwaes_val = (otp.platform_cfg.share_cache_ram_reg << 1) | otp.platform_cfg.share_dsp_ram_reg;

    HWAES->DSP_CACHE_MEM_CTRL = get_secure_reg_cfg_val(hwaes_val);
}
