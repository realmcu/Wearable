...
T_SD_STATUS sd_if_bind(T_SD_IF_TYPE sd_if_type, T_SD_IF *p_sd_if, void **pp_dev_info)
{
    ...
    if (p_sd_if->use_flag == true)
    {
        ...
        {
            //SDIO_PRINT_ERROR1("sd_if_bind: sd_if_type violation = %d", sd_if_type);
...
//        MMC_RegisterFops(&(p_sd_if->p_prot_fops));
    ...
    {
        //SDIO_PRINT_ERROR1("sd_if_bind: invalid sd_if_type = %d",  sd_if_type);
...
fail:
    //SDIO_PRINT_ERROR2("sd_if_init: status = %d, fail line %d", ret, fail_line);
    return ret;
}
...
T_SD_STATUS sd_if_read(uint32_t sector, uint32_t buf, uint16_t blk_size, uint16_t blk_num)
{
    ...
    do
    {
        ...
        if (ret != SD_OK)
        {
            //SDIO_PRINT_ERROR1("sd_if_read: err status = 0x%x", ret);
            return ret;
        }
        ...
        if (ret != SD_OK)
        {
            //SDIO_PRINT_ERROR1("sd_if_read: err ret = %d", ret);
...
T_SD_STATUS sd_if_write(uint32_t sector, uint32_t buf, uint16_t blk_size, uint16_t blk_num)
{
    ...
    do
    {
        ...
        if (ret != SD_OK)
        {
            //SDIO_PRINT_ERROR1("sd_if_write: err status = 0x%x", ret);
            return ret;
        }
        ...
        if (ret != SD_OK)
        {
            //SDIO_PRINT_ERROR1("sd_if_write: err ret = %d", ret);
            ret = SDIO_CheckErr_TryRecovery();
        }
    }
    while (--retry && (ret == SD_CMD_LINE_RECOVERABLE_ERROR || ret == SD_DAT_LINE_RECOVERABLE_ERROR));
...
T_SD_STATUS sd_if_erase(uint32_t start_addr, uint32_t end_addr)
{
    ...
    while (retry--)
    {
        ...
        {
            //SDIO_PRINT_INFO1("sd_if_erase: ret = 0x%x", 1, ret);
            break;
        }
    }
#endif
...
fail:
    //SDIO_PRINT_ERROR2("sd_if_erase: err status = 0x%x, fail line %d", ret, fail_line);
    return ret;
}
...
fail:
    //SDIO_PRINT_ERROR2("sd_if_async_read: err status = 0x%x, fail line %d", ret, fail_line);
    return ret;
}
...
T_SD_STATUS sd_if_async_write(uint64_t sector, uint32_t buf, uint16_t blk_size, uint16_t blk_num)
{
    ...
    return ret;
fail:
    //SDIO_PRINT_ERROR2("sd_if_async_write: err status = 0x%x, fail line %d", ret, fail_line);
    return ret;
}
#endif
...
uint64_t sd_if_get_dev_capacity(void)
{
    ...
    //SDIO_PRINT_INFO3("sd_if_get_dev_capacity: 0x%x, 0x%x, sd_if_type = 0x%x",
...
uint32_t sd_if_get_dev_block_num(void)
{
    ...
    //SDIO_PRINT_INFO1("sd_if_get_dev_block_num: 0x%x", card_blk_cnt);
    return card_blk_cnt;
}
...
uint32_t sd_if_get_dev_block_size(void)
{
    ...
    //SDIO_PRINT_INFO2("sd_if_get_dev_block_size: 0x%x, sd_if_type = 0x%x", block_size, sd_if_type);
...
T_SD_IF_TYPE sd_if_get_dev_type(void)
{
    ...
    //SDIO_PRINT_INFO1("sd_if_get_type: 0x%x", sd_if_type);
    return sd_if_type;
}
