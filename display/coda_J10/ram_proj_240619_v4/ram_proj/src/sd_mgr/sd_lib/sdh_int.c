...
{
    //SDIO_PRINT_INFO1("SDH_TransCtrlIntConfig: status = 0x%x", newState);
...
void  SDIO_IrqNorIntCback(T_SDH_NOR_INT_STATUS *int_status)
{
    ...
    {
        //SDIO_PRINT_ERROR0("SDIO_Intr_Handler: detect error status");
        return;
    }
}
...
void SDIO_HOST_Handler(void)
{
    ...
    //SDIO_PRINT_INFO0("SDIO_HOST_Handler");
    ...
    if (err_int_status.d16)
    {
        ...
        DBG_DIRECT("err_int_status.d16: 0x%x", err_int_status.d16);
...
T_SD_STATUS SDIO_CheckErr_TryRecovery(void)
{
    ...
    if (sdio_err_status & 0xf) // D03-00 is set
    {
        ...
        if (retry == 0)
        {
            ...
            DBG_DIRECT("SD_NON_RECOVERABLE_ERROR 1");
            goto exit_recovery;
        }
    ...
    if (sdio_err_status & 0x70) // D06-04 is set
    {
        ...
        {
            ret = SD_NON_RECOVERABLE_ERROR;
            DBG_DIRECT("SD_NON_RECOVERABLE_ERROR 2");
            goto exit_recovery;
        }
    ...
    {
        ret = SD_NON_RECOVERABLE_ERROR;
        DBG_DIRECT("SD_NON_RECOVERABLE_ERROR 3");
        goto exit_recovery;
    }
    ...
    {
        ret = SD_NON_RECOVERABLE_ERROR;
        DBG_DIRECT("SD_NON_RECOVERABLE_ERROR 4");
        goto exit_recovery;
    }
    ...
    {
        ret = SD_NON_RECOVERABLE_ERROR;
        DBG_DIRECT("SD_NON_RECOVERABLE_ERROR 5");
        DBG_DIRECT("SDIO->PRESENT_STATE: 0x%x", SDIO->PRESENT_STATE);
        goto exit_recovery;
    }
