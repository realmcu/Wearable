...
{
    //115200 default(start bit + 8 data bit + stop bit)
    UART_InitStruct->UART_Div                 = 0x38;
    UART_InitStruct->UART_Ovsr                = 0x1;
    UART_InitStruct->UART_OvsrAdj             = 0x3;
...
void UART_INTConfig_rom(UART_TypeDef *UARTx, uint32_t UART_IT, FunctionalState newState)
{
    ...
    if (UART_IT & 0x3F)
    {
        ...
        if (newState == ENABLE)
        {
            ...
            UARTx->UART_DLM_IER |= UART_IT;
        }
        else
        {
            /* Disable the selected UARTx interrupts */
            UARTx->UART_DLM_IER &= (uint32_t)~UART_IT;
        }
    }
    return;
}
