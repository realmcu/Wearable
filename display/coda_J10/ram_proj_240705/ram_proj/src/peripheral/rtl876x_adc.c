...
extern void  ADC_DelayStructConfig(ADC_InitTypeDef *ADC_InitStruct);
...
#endif
    ADC_DelayStructConfig(ADC_InitStruct);
    return;
}
...
uint16_t ADC_ReadAvgRawData(ADC_TypeDef *ADCx)
{
    ...
    uint16_t data = 0;
    data = (uint16_t)((ADCx->ADC_SCHD0 >> 2) & 0xFFF);
...
void ADC_SchTableConfig(ADC_TypeDef *ADCx, uint16_t Index, uint8_t AdcMode)
{
    ...
    if (index < 16)
    {
        ...
        if (Index & BIT0)
        {
            *(uint32_t *)((uint32_t *)(&ADCx->ADC_SCHTAB0) + (Index >> 1)) &= ~(0x1f << 16);
            *(uint32_t *)((uint32_t *)(&ADCx->ADC_SCHTAB0) + (Index >> 1)) |= (AdcMode << 16);
        }
        else
        {
            *(uint32_t *)((uint32_t *)(&ADCx->ADC_SCHTAB0) + (Index >> 1)) &= ~(0x1f);
