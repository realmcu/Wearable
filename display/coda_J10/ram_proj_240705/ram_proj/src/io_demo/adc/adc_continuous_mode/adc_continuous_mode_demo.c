...
void *adc_test_timer_handle = NULL;
void adc_timer_finish_handler(void *xTimer);
...
static void adc_gdma_init(void)
{
    ...
    {
        DBG_DIRECT("adc dma request fail!");
        return;
    }
...
static void driver_adc_deinit(void)
{
    ...
    {
        APP_PRINT_ERROR0("Error: adc_test_timer Stop Fail.");
    }

    /* Delete adc_test_timer */
    if (!os_timer_delete(&adc_test_timer_handle))
    {
        APP_PRINT_ERROR0("Error: adc_test_timer Delete Fail.");
    }
    else
    {
...
static void adc_test_timer_init(void)
{
    ...
    if (adc_test_timer_handle == NULL)
    {
        ...
        {
            APP_PRINT_ERROR0("Error: adc_test_timer create Fail.");
        }
    }
}
...
void ADC_Handler(void)
{
    ...
    {
        APP_PRINT_ERROR0("ADC_INT_FIFO_RD_ERR");
...
void adc_dma_handler(void)
{
    ...
    for (i = 0; i < adc_dma_count; i++)
    {
        ...
        APP_PRINT_TRACE4("raw data[%d]:%d,result[%d]:%d", i, adc_dma_buffer[i], i, result[i]);
    }
    ...
    {
        APP_PRINT_ERROR0("Error: adc_test_timer Start Fail.");
    }
}
...
void adc_timer_finish_handler(void *xTimer)
{
    APP_PRINT_TRACE0("adc_timer_finish_handler");
