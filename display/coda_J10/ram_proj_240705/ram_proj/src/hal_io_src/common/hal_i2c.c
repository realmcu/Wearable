...
#define IS_INVALID_DMA_CH_NUM(ch_num)       ((ch_num) >= 9)
#define I2C_STATUS_TO_HAL_RET(status)       ((T_I2C_STATUS)(status + I2C_STATUS_ARB_LOST - 1))
...
{
    IO_PRINT_ERROR3("hal i2c fail, func %s, id %x, status %x", TRACE_STRING(func), i2c_id, status);
...
T_I2C_STATUS hal_i2c_init_master_mode(T_I2C_ID i2c_id, const T_I2C_MASTER_CONFIG *i2c_config)
{
    ...
    i2c_param.I2C_AddressMode = i2c_config->addr_mode;
                                 ...
                                 uint32_t read_len)
{
    ...
    status = I2C_MasterWrite(g_i2c_regbase[i2c_id], read_buf, read_len);
