...
PPE_ERR PPE_blend(ppe_buffer_t *image, ppe_buffer_t *buffer, PPE_translate_t *trans)
{
    ...
    PPE_rect_t transfer_rect = {.left = 0, .right = image->width - 1, \
    ...
    {
        PPE_Input_Layer1.start_x = 0;
        transfer_rect.left = -trans->x;
    }
    if ((trans->x + image->width) > buffer->width)
    {
        transfer_rect.right = buffer->width - trans->x - 1;
    }
    if (trans->y < 0)
    {
        PPE_Input_Layer1.start_y = 0;
        transfer_rect.top = -trans->y;
    }
    if ((trans->y + image->height) > buffer->height)
    {
        transfer_rect.bottom = buffer->height - trans->y - 1;
    }
    ...
    PPE_Input_Layer1.hw_index                       = 0;
    PPE_InitInputLayer(2, &PPE_Input_Layer1);

    /*initial input layer2*/
    PPE_Input_Layer2.src_addr                       = (uint32_t)buffer->memory;
    PPE_Input_Layer2.start_x                        = 0;
    PPE_Input_Layer2.start_y                        = 0;
    PPE_Input_Layer2.width                          = buffer->width;
    PPE_Input_Layer2.height                         = buffer->height;
    PPE_Input_Layer2.const_ABGR8888_value           = 0XFFAABBCC;
    PPE_Input_Layer2.format                         = buffer->format;
    ...
    PPE_Input_Layer2.key_color_value                = buffer->color_key_value;
