...
static void st7701s_dma_init(uint8_t *init_buffer)
{
    ...
    LCDC_SET_GROUP1_BLOCKSIZE(ST7701S_320800_LCD_WIDTH * ST7701S_DRV_PIXEL_BITS / 8);
    LCDC_SET_GROUP2_BLOCKSIZE(ST7701S_320800_LCD_WIDTH * ST7701S_DRV_PIXEL_BITS / 8);

    LCDC_DMALLI_InitTypeDef LCDC_DMA_LLI_Init = {0};

    LCDC_DMA_LLI_Init.g1_source_addr = (uint32_t)init_buffer;
    LCDC_DMA_LLI_Init.g2_source_addr = (uint32_t)((uint32_t)init_buffer + ST7701S_320800_LCD_WIDTH *
                                                  ST7701S_DRV_PIXEL_BITS / 8);
    LCDC_DMA_LLI_Init.g1_sar_offset = ST7701S_320800_LCD_WIDTH * ST7701S_DRV_PIXEL_BITS / 8 * 2;
    LCDC_DMA_LLI_Init.g2_sar_offset = ST7701S_320800_LCD_WIDTH * ST7701S_DRV_PIXEL_BITS / 8 * 2;

    LCDC_DMA_Infinite_Buf_Update((uint8_t *)init_buffer,
                                 (uint8_t *)init_buffer + ST7701S_320800_LCD_WIDTH *
                                 ST7701S_DRV_PIXEL_BITS / 8);
    ...
    LCDC_SetTxPixelLen(ST7701S_320800_LCD_WIDTH * ST7701S_320800_LCD_HEIGHT);
...
void rtk_lcd_hal_init(void)
{
    ...
    uint32_t HSA = 3, HFP = 20, HBP = 50, HACT = ST7701S_320800_LCD_WIDTH;
    uint32_t VSA = 6, VFP = 25, VBP = 40, VACT = ST7701S_320800_LCD_HEIGHT;
    ...
    //*******************************/
#include "st7701s_rgb565.txt"
    uint8_t *pixel = (uint8_t *)SPIC1_ADDR;

    for (uint32_t i = 0; i < ST7701S_320800_LCD_WIDTH * ST7701S_320800_LCD_HEIGHT * 3; i = i + 3)
    {
        pixel[i] = 0xFF;
        pixel[i + 1] = 0x00;
        pixel[i + 2] = 0x00;
    ...
    rtk_lcd_hal_update_framebuffer((uint8_t *)SPIC1_ADDR,
                                   ST7701S_320800_LCD_WIDTH * ST7701S_320800_LCD_HEIGHT);
...
void rtk_lcd_hal_update_framebuffer(uint8_t *buf, uint32_t len)
{
    ...
    else
    {
        LCDC_DMA_Infinite_Buf_Update(buf,
                                     (uint32_t)buf + ST7701S_320800_LCD_WIDTH * ST7701S_DRV_PIXEL_BITS / 8);
    }
}
...
{
    return ST7701S_320800_LCD_WIDTH;
}

uint32_t rtk_lcd_hal_get_height(void)
{
    return ST7701S_320800_LCD_HEIGHT;
}
