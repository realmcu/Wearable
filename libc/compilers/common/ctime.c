...
rt_align(RT_ALIGN_SIZE) static const char *months =
...
static void rtthread_timer_wrapper(void *timerobj)
{
    ...
    timer->reload = ((timer->interval.tv_sec * NANOSECOND_PER_SECOND + timer->interval.tv_nsec) *
                     RT_KTIME_RESMUL) /
    ...
    struct lwp_timer_event_param *data = rt_container_of(timer->work, struct lwp_timer_event_param,
                                                         work);
...
int timer_create(clockid_t clockid, struct sigevent *evp, timer_t *timerid)
{
    ...
    rt_ktime_hrtimer_init(&timer->hrtimer, timername, 0,
...
int timer_gettime(timer_t timerid, struct itimerspec *its)
{
    ...
    if (timer->status == ACTIVE)
    {
        ...
        nanoseconds = ((remain_cnt - rt_ktime_cputimer_getcnt()) * rt_ktime_cputimer_getres()) /
                      RT_KTIME_RESMUL;
                  ...
                  struct itimerspec *ovalue)
{
    ...
    int64_t ns = value->it_value.tv_nsec - ts.tv_nsec + (value->it_value.tv_sec - ts.tv_sec) *
