...
/** @defgroup  ADC_CHARGER_DEMO ADC Charger Demo
    ...
    * @{
    */
void *io_demo_app_task_handle;
void *io_evt_queue_handle;
...
ADC_CHARGER_DATA_MANAGER adc_charger_data_mgr;
...
static void app_adc_vbat_ntc_voltage_read_callback(void *pvPara, uint32_t int_status)
{
    ...
    APP_PRINT_INFO3("temperature_battery_1:%d, temperature_battery_2:%d, voltage_battery:%d",
...
{
    APP_PRINT_INFO1("app_adc_vbat_ntc_voltage_init: %d", adc_vbat_ntc_voltage_init);
    ...
    {
        APP_PRINT_ERROR0("app_adc_vbat_ntc_voltage_init: adc_mgr_register_req failed");
        return;
    }
...
{
    APP_PRINT_INFO2("app_adc_vbat_ntc_voltage_deinit: %d, %d", adc_vbat_ntc_voltage_init,
                    adc_channel_vbat_ntc_voltage);
...
{
    APP_PRINT_INFO1("app_adc_vbat_ntc_voltage_read: %d", adc_vbat_ntc_voltage_init);
...
static void io_demo_task(void *param)
{
    ...
    APP_PRINT_INFO0("io_demo_task running !");
    adc_charger_demo();
    ...
    while (true)
    {
        ...
        {
            APP_PRINT_INFO1("event =0x%x!", event);
