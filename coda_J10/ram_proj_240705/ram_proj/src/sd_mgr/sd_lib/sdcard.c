...
T_SD_STATUS SD_ResetCard(void)
{
    ...
    {
        sd_print_error("SD_ResetCard: error ret = SD_CMD_INHIBIT_BEFORE_RESET ");
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        sd_print_error("SD_ResetCard: error ret = SD_SDIO_CMD_INHIBIT_AFTER_RESET ");
...
static T_SD_STATUS SD_CheckResp1(uint32_t cmd)
{
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        sd_print_error("SD_CheckResp1: error = SD_SDIO_CMD_INHIBIT_AFTER_RESET ");
        return SD_SDIO_CMD_INHIBIT_AFTER_RESET;
    }
    ...
    {
        sd_print_error("SD_CheckResp1: cmd error = SD_CMD_ERROR ");
        return SD_CMD_ERROR;
    }
    ...
    {
        sd_print_error("SD_CheckResp1: response error = SD_ADDR_OUT_OF_RANGE ");
        return SD_ADDR_OUT_OF_RANGE;
    }

    if (response & SD_OCR_ADDR_MISALIGNED)
    {
        sd_print_error("SD_CheckResp1: response error = SD_ADDR_MISALIGNED ");
        return SD_ADDR_MISALIGNED;
    }

    if (response & SD_OCR_BLOCK_LEN_ERR)
    {
        sd_print_error("SD_CheckResp1: response error = SD_BLOCK_LEN_ERR ");
        return SD_BLOCK_LEN_ERR;
    }

    if (response & SD_OCR_ERASE_SEQ_ERR)
    {
        sd_print_error("SD_CheckResp1: response error = SD_ERASE_SEQ_ERR ");
        return SD_ERASE_SEQ_ERR;
    }

    if (response & SD_OCR_BAD_ERASE_PARAM)
    {
        sd_print_error("SD_CheckResp1: response error = SD_BAD_ERASE_PARAM ");
        return SD_BAD_ERASE_PARAM;
    }

    if (response & SD_OCR_WRITE_PROT_VIOLATION)
    {
        sd_print_error("SD_CheckResp1: response error = SD_WRITE_PROT_VIOLATION ");
        return SD_WRITE_PROT_VIOLATION;
    }

    if (response & SD_OCR_LOCK_UNLOCK_FAILED)
    {
        sd_print_error("SD_CheckResp1: response error = SD_LOCK_UNLOCK_FAILED ");
        return SD_LOCK_UNLOCK_FAILED;
    }

    if (response & SD_OCR_COM_CRC_FAILED)
    {
        sd_print_error("SD_CheckResp1: response error = SD_COM_CRC_FAILED ");
        return SD_COM_CRC_FAILED;
    }

    if (response & SD_OCR_ILLEGAL_CMD)
    {
        sd_print_error("SD_CheckResp1: response error = SD_ILLEGAL_CMD ");
        return SD_ILLEGAL_CMD;
    }

    if (response & SD_OCR_CARD_ECC_FAILED)
    {
        sd_print_error("SD_CheckResp1: response error = SD_CARD_ECC_FAILED ");
        return SD_CARD_ECC_FAILED;
    }

    if (response & SD_OCR_CC_ERROR)
    {
        sd_print_error("SD_CheckResp1: response error = SD_CC_ERROR ");
        return SD_CC_ERROR;
    }

    if (response & SD_OCR_GENERAL_UNKNOWN_ERROR)
    {
        sd_print_error("SD_CheckResp1: response error = SD_GENERAL_UNKNOWN_ERROR ");
        return SD_GENERAL_UNKNOWN_ERROR;
    }

    if (response & SD_OCR_CID_CSD_OVERWRIETE)
    {
        sd_print_error("SD_CheckResp1: response error = SD_CID_CSD_OVERWRITE ");
        return SD_CID_CSD_OVERWRITE;
    }

    if (response & SD_OCR_WP_ERASE_SKIP)
    {
        sd_print_error("SD_CheckResp1: response error = SD_WP_ERASE_SKIP ");
        return SD_WP_ERASE_SKIP;
    }

    if (response & SD_OCR_CARD_ECC_DISABLED)
    {
        sd_print_error("SD_CheckResp1: response error = SD_CARD_ECC_DISABLED ");
        return SD_CARD_ECC_DISABLED;
    }

    if (response & SD_OCR_ERASE_RESET)
    {
        sd_print_error("SD_CheckResp1: response error = SD_ERASE_RESET ");
        return SD_ERASE_RESET;
    }

    if (response & SD_OCR_AKE_SEQ_ERROR)
    {
        sd_print_error("SD_CheckResp1: response error = SD_AKE_SEQ_ERROR ");
        return SD_AKE_SEQ_ERROR;
    }
    /*  The card will expect ACMD */
    if (!(response & SD_OCR_SD_APP_CMD))
    {
        sd_print_error("SD_CheckResp1: response error = SD_APP_CMD ");
        return SD_APP_CMD;
    }
...
T_SD_STATUS SD_GetCardInfo(void *p_info)
{
    ...
    sd_print_info("SD_GetCardInfo: DeviceSize 0x%x, DeviceSizeMul 0x%x, RdBlockLen 0x%x, capacity 0x%x, block_size 0x%x",
...
T_SD_STATUS SD_SendIfCond(void)
{
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        sd_print_error("SD_SendIfCond: SD_SDIO_CMD_INHIBIT");
        return SD_SDIO_CMD_INHIBIT;
    }
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        sd_print_error("SD_SendIfCond: SD_SDIO_CMD_INHIBIT_AFTER_RESET");
        return SD_SDIO_CMD_INHIBIT_AFTER_RESET;
    }
...
T_SD_STATUS SD_SendOpCond(void)
{
    ...
    p_sd_host_prop->SDH_GetResponse(SDH_RSP0);
    ...
    while (--retry)
    {
        ...
        if (p_sd_host_prop->SDH_GetResponse(SDH_RSP0) & BIT(31))
        {
            if (p_sd_host_prop->SDH_GetResponse(SDH_RSP0) & BIT30)
            {
                ...
                sd_print_info("SD_SendOpCond: SD type = SD_TYPE_SDHC_OR_SDXC");
            }
            else
            {
                SD_Type = SD_TYPE_SDSC;
                sd_print_info("SD_SendOpCond: SD type = SD_TYPE_SDSC");
            }
            break;
        }
        p_sd_host_prop->SDH_GetResponse(SDH_RSP2);
...
fail:
    sd_print_error("SD_SendOpCond: SD_SDIO_CMD_INHIBIT, fail line %d", fail_line);
    return SD_SDIO_CMD_INHIBIT;
                                ...
                                bool switch_fun)
{
    ...
    {
        sd_print_info("SD_SetHighSpeedMode: not support cmd6");
        return SD_OK;
    }
    ...
    {
#ifdef TARGET_RTL87X3E
        pbuf = os_mem_alloc(RAM_TYPE_BUFFER_ON, 512 / 8); // cmd 6 will return 512 bits data
#else
        pbuf = pvPortMalloc(512 / 8); // cmd 6 will return 512 bits data
#endif

        if (pbuf == NULL)
        {
            sd_print_error("SD_SetHighSpeedMode: os_mem_alloc fail");
            return SD_NO_FREE_SPACE;  // swicthing to high speed mode is not mandatory
        }
    }

    SDH_SetErrorInt(false);
    ...
    if ((pbuf[16] & 0xf) != 0x1)
    {
        sd_print_info("SD_SetHighSpeedMode: not support high speed mode");
        goto clean_up;
    }
...
clean_up:
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_SetHighSpeedMode: err sd_status %d, line %d", sd_status, fail_line);
    }

    T_SDH_ERR_INT_STATUS err_int_status = {.d16 = SDIO_GetAllErrINTStatus()};
    if (err_int_status.d16)
    {
        sd_status = SD_ERROR_STATUS;
        SDIO_ErrStatusClearAndRecovery();
    }

    SDH_SetErrorInt(true);
...
/**
  * @brief  Tuning bus delay register by CMD6 mode0.
  * @retval T_SD_STATUS: SD status.
  */
T_SD_STATUS SD_SetBusDelay(void *p_dev)
{
    T_SD_STATUS sd_status = SD_OK;
    T_SD_DEV_INFO *p_dev_info = (T_SD_DEV_INFO *)p_dev;
    uint8_t bus_delay = 0;
    uint8_t bus_delay_pass_cnt = 0;
    uint8_t bus_delay_unpass_cnt = 0;
    uint8_t bus_delay_pass_over_cnt = 0;
    uint8_t bus_delay_pass[16] = {0};
    uint8_t bus_delay_unpass[16] = {0};

    while (bus_delay <= 15)
    {
        sd_status = p_sd_host_prop->SDH_BusDelaySel(sd_cfg.sdh_bus_width, bus_delay);
        if (sd_status != SD_OK)
        {
            sd_print_error("SD_SetBusDelay: SDH_BusDelaySel error, sd_status:%d", sd_status);
        }

        sd_status = SD_SetHighSpeedMode(ENABLE, p_dev_info->CSD_Tab, false);

        if (sd_status != SD_OK)
        {
            sd_print_error("SD_SetBusDelay: SD_SetHighSpeedMode error, sd_status:%d, bus_delay:%d",
                           sd_status, bus_delay);

            bus_delay_unpass[bus_delay_unpass_cnt] = bus_delay;
            bus_delay_unpass_cnt++;
        }
        else
        {
            bus_delay_pass[bus_delay_pass_cnt] = bus_delay;
            bus_delay_pass_cnt++;
        }
        bus_delay++;
    }
    ...
    {
        if ((bus_delay_unpass_cnt != 0) && (bus_delay_pass[i] > bus_delay_unpass[bus_delay_unpass_cnt - 1]))
        {
            bus_delay_pass_over_cnt++;
        }
    }

    bus_delay_pass_cnt = bus_delay_pass_cnt - bus_delay_pass_over_cnt;
    bus_delay = bus_delay_pass[bus_delay_pass_cnt / 2];

    sd_print_info("SD_SetBusDelay: bus_delay_pass_cnt = %d, bus_delay = %d",
                  bus_delay_pass_cnt, bus_delay);

    p_sd_host_prop->SDH_BusDelaySel(sd_cfg.sdh_bus_width, bus_delay);
...
T_SD_STATUS SD_InitCard(void *p_dev)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_ResetCard error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: CMD8 reponse error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_SendOpCond CMD41 error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_GetCID error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_GetRCA error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_GetCSD error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_SendCID error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_InitCard: SD_SelectCmd error, sd_status %d", sd_status);
        return sd_status;
    }
    ...
    while (bus_delay <= 15)
    {
        ...
        if (sd_status != SD_OK)
        {
            sd_print_error("SD_InitCard: SDH_BusDelaySel error, sd_status:%d", sd_status);
        }

        sd_status = SD_SetHighSpeedMode(ENABLE, p_dev_info->CSD_Tab, true);

        if (sd_status != SD_OK)
        {
            sd_print_error("SD_InitCard: SD_SetHighSpeedMode error, sd_status:%d, bus_delay:%d",
                           sd_status, bus_delay);
        }
        else
        {
    ...
    sd_print_info("SD_InitCard: bus_delay = %d", bus_delay);
    ...
    if (sd_cfg.sdh_bus_width == SD_BUS_WIDTH_4B)
    {
        ...
        if (sd_status != SD_OK)
        {
            sd_print_error("SD_InitCard: SD_SendAppCmd error, sd_status %d", sd_status);
            return sd_status;
        }
        ...
        if (sd_status != SD_OK)
        {
            sd_print_error("SD_InitCard: SD_SetBusWide error, sd_status %d", sd_status);
            return sd_status;
        }
    }

    p_dev_info->initialized = true;
    sd_print_info("SD_InitCard: status: SD_OK");
    return sd_status;
}
...
T_SD_STATUS SD_SetBlockLen(uint32_t block_len)
{
    ...
    if (SD_TYPE_SDSC == SD_Type)
    {
        ...
        if (sd_status != SD_OK)
        {
            sd_print_error("SD_SetBlockLen: sd_status %d, line %d", sd_status, __LINE__);
            return sd_status;
        }
    }
    return sd_status;
...
fail:
    sd_print_error("SD_ReadSingleBlock: err sd_status %d, line %d", sd_status, fail_line);
    return sd_status;
}
...
fail:
    sd_print_error("SD_ReadMultiBlock: err sd_status %d, line %d", sd_status, fail_line);
    return sd_status;
}
...
T_SD_STATUS SD_WriteSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    {
        sd_print_error("SD_WriteSingleBlock: err rsp = sd_write_protection_violation");
        return SD_WRITE_PROT_VIOLATION;
    }
    return sd_status;

fail:
    sd_print_error("SD_WriteSingleBlock: err sd_status %d, line %d", sd_status, fail_line);
    return sd_status;
}
                               ...
                               uint16_t block_count)
{
    ...
    if (p_sd_host_prop->SDH_GetResponse(SDH_RSP0) & SD_OCR_WRITE_PROT_VIOLATION)
    {
        sd_print_error("SD_WriteMultiBlock: err res = sd_write_protection_violation");
        return SD_WRITE_PROT_VIOLATION;
    }
    return sd_status;

fail:
    sd_print_error("SD_WriteMultiBlock: err sd_status %d, line %d", sd_status, fail_line);
    return sd_status;
}
...
T_SD_STATUS SD_Earse(uint32_t start_addr, uint32_t end_addr)
{
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        sd_print_error("SD_Earse: CMD line is not idle");
        return SD_SDIO_CMD_INHIBIT;
    }
    return sd_status;

fail:
    sd_print_error("SD_Earse: err sd_status %d, line %d", sd_status, fail_line);
    return sd_status;
}
...
T_SD_STATUS SD_StopTransfer(void)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_StopTransfer: sd_status %d, line %d", sd_status, __LINE__);
    }
    return sd_status;
}
...
T_SD_STATUS SD_AsyncReadSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_AsyncReadSingleBlock: SDH_DataConfig, err sd_status %d, line %d", sd_status,
                       __LINE__);
        return sd_status;
    }
                                   ...
                                   uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_AsyncReadMultiBlock: SDH_DataConfig, err sd_status %d, line %d", sd_status,
                       __LINE__);
        return sd_status;
    }
...
T_SD_STATUS SD_AsyncWriteSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_AsyncWriteSingleBlock: SDH_DataConfig, err sd_status %d, line %d",
                       sd_status,
                       __LINE__);
        return sd_status;
    }
                                    ...
                                    uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        sd_print_error("SD_AsyncWriteMultiBlock: SDH_DataConfig, err sd_status %d, line %d", sd_status,
                       __LINE__);
        return sd_status;
    }
