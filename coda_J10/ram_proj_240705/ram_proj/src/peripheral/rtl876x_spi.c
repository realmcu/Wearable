...
#else
#define SPI_NOT_SLAVE(SPIx)          (SPIx != SPI_SLAVE)
...
void SPI_Master_Init(SPI_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));

    /* Disable SPI device before change configuration */
    SPI_ENR_TypeDef spi_0x08 = {.d32 = SPIx->SPI_ENR};
    spi_0x08.b.SPI_EN = 0;
    SPIx->SPI_ENR = spi_0x08.d32;
    ...
    if (IS_SPIM_PERIPH(SPIx))
    {
        ...
        SPI_CTRL0_TypeDef spi_0x00 = {.d32 = SPIx->SPI_CTRL0};
        ...
        {
            SPI_CTRL1_TypeDef spi_0x04 = {.d32 = SPIx->SPI_CTRL1};
            if (SPI_InitStruct->SPI_RXNDF > 0)
            {
                spi_0x04.b.NDF = SPI_InitStruct->SPI_RXNDF - 1;
            }
            SPIx->SPI_CTRL1 = spi_0x04.d32;
        }
...
void SPI_Master_JDI_Init(SPI_JDI_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));

    /* Disable SPI device before change configuration */
    SPI_JDI_ENR_TypeDef spi_0x08 = {.d32 = SPIx->SPI_ENR};
    spi_0x08.b.SPI_EN = 0;
    SPIx->SPI_ENR = spi_0x08.d32;

    /* Configure SPI Master mode if select SPI0, SPI1, SPI2 or SPI3 */
    if (IS_SPIM_JDI_PERIPH((SPI_TypeDef *)SPIx))
    {
#if (SPI_SUPPORT_APH_BRIDGE_FOR_HIGH_SPEED == 1)
        SPI_ConfigAPHBridge((SPI_TypeDef *)SPIx);
#endif

#if (SPI1_SUPPORT_HS == 1)
        if ((SPI_TypeDef *)SPIx == SPI1)
        {
            SPI_SPI1AHBBridgeConfig(DISABLE);
        }
        else if ((SPI_TypeDef *)SPIx == SPI1_HS)
        {
        ...
        /* Configure SPI master mode parameters */
        SPI_JDI_CTRL0_TypeDef spi_0x00 = {.d32 = SPIx->SPI_CTRL0};
        spi_0x00.b.FRF = SPI_InitStruct->SPI_FrameFormat;
        spi_0x00.b.SCPH = SPI_InitStruct->SPI_CPHA;
        spi_0x00.b.SCPOL = SPI_InitStruct->SPI_CPOL;
        spi_0x00.b.TMOD = SPI_InitStruct->SPI_Direction;
        ...
        /* Set master clock divider */
        SPI_JDI_BAUDR_TypeDef spi_0x14 = {.d32 = SPIx->SPI_BAUDR};
        spi_0x14.b.SCKDV = SPI_InitStruct->SPI_BaudRatePrescaler % 0xFFFF;
        SPIx->SPI_BAUDR = spi_0x14.d32;

        /* Enable slave Select function in master mode */
        SPI_JDI_SER_TypeDef spi_0x10 = {.d32 = SPIx->SPI_SER};
        spi_0x10.b.SER = 1;
        SPIx->SPI_SER = spi_0x10.d32;
        ...
        if ((SPI_InitStruct->SPI_Direction == SPI_Direction_RxOnly) || \
            (SPI_InitStruct->SPI_Direction == SPI_Direction_EEPROM))
        {
            SPI_JDI_CTRL1_TypeDef spi_0x04 = {.d32 = SPIx->SPI_CTRL1};
            if (SPI_InitStruct->SPI_RXNDF > 0)
            {
                spi_0x04.b.NDF = SPI_InitStruct->SPI_RXNDF - 1;
            }
    ...
    /* Set Tx empty level */
    SPI_JDI_TXFTLR_TypeDef spi_0x18 = {.d32 = SPIx->SPI_TXFTLR};
    spi_0x18.b.TFT = SPI_InitStruct->SPI_TxThresholdLevel;
    SPIx->SPI_TXFTLR = spi_0x18.d32;

    /*set Rx full level*/
    SPI_JDI_RXFTLR_TypeDef spi_0x1c = {.d32 = SPIx->SPI_RXFTLR};
    spi_0x1c.b.RFT = SPI_InitStruct->SPI_RxThresholdLevel;

    /* Config SPI dma mode */
    SPI_JDI_DMACR_TypeDef spi_0x4c = {.d32 = SPIx->SPI_DMACR};
    spi_0x4c.b.RDMAE = SPI_InitStruct->SPI_RxDmaEn;
    spi_0x4c.b.TDMAE = SPI_InitStruct->SPI_TxDmaEn;
    SPIx->SPI_DMACR = spi_0x4c.d32;

    /* Config SPI waterlevel */
    SPI_JDI_DMATDLR_TypeDef spi_0x50 = {.d32 = SPIx->SPI_DMATDLR};
    SPI_JDI_DMARDLR_TypeDef spi_0x54 = {.d32 = SPIx->SPI_DMARDLR};
    spi_0x50.b.DMATDL = SPI_InitStruct->SPI_TxWaterlevel;
    spi_0x54.b.DMARDL = SPI_InitStruct->SPI_RxWaterlevel;
    SPIx->SPI_DMATDLR = spi_0x50.d32;
...
void SPI_Slave_Init(SPI_Slave_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));

    /* Disable SPI device before change configuration */
    SPI_SLV_ENR_TypeDef spi_0x08 = {.d32 = SPIx->SPI_SLV_ENR};
    spi_0x08.b.SPI_EN = 0;
    SPIx->SPI_SLV_ENR = spi_0x08.d32;

#if !SPI0_SUPPORT_MASTER_SLAVE
    /* Configure SPI Slave mode if select SPI_SLAVE */
    if ((SPI_TypeDef *)SPIx == SPI_SLAVE)
    {
...
#endif
    /* Set Tx empty level */
    SPI_SLV_TXFTLR_TypeDef spi_0x18 = {.d32 = SPIx->SPI_SLV_TXFTLR};
    spi_0x18.b.TFT = SPI_InitStruct->SPI_TxThresholdLevel;
    SPIx->SPI_SLV_TXFTLR = spi_0x18.d32;

    /*set Rx full level*/
    SPI_SLV_RXFTLR_TypeDef spi_0x1c = {.d32 = SPIx->SPI_SLV_RXFTLR};
    spi_0x1c.b.RFT = SPI_InitStruct->SPI_RxThresholdLevel;

    /* Config SPI dma mode */
    SPI_SLV_DMACR_TypeDef spi_0x4c = {.d32 = SPIx->SPI_SLV_DMACR};
    spi_0x4c.b.RDMAE = SPI_InitStruct->SPI_RxDmaEn;
    spi_0x4c.b.TDMAE = SPI_InitStruct->SPI_TxDmaEn;
    SPIx->SPI_SLV_DMACR = spi_0x4c.d32;

    /* Config SPI waterlevel */
    SPI_SLV_DMATDLR_TypeDef spi_0x50 = {.d32 = SPIx->SPI_SLV_DMATDLR};
    SPI_SLV_DMARDLR_TypeDef spi_0x54 = {.d32 = SPIx->SPI_SLV_DMARDLR};
    spi_0x50.b.DMATDL = SPI_InitStruct->SPI_TxWaterlevel;
    spi_0x54.b.DMARDL = SPI_InitStruct->SPI_RxWaterlevel;
    SPIx->SPI_SLV_DMATDLR = spi_0x50.d32;

    if (SPI_InitStruct->SPI_RxDmaEn)
    {
        /* DMA mode should config DMARDLR later if open AHB */
        SPIx->SPI_SLV_RXFTLR = spi_0x1c.d32;
        SPIx->SPI_SLV_DMARDLR = spi_0x54.d32;
    }
    else
    {
        /* CPU mode should config RXFTLR later if open AHB */
...
void SPI_Init(SPI_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_Init((SPI_JDI_TypeDef *)SPIx, SPI_InitStruct);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_Init((SPI_Slave_TypeDef *)SPIx, SPI_InitStruct);
    }
    else
    {
        SPI_Master_Init(SPIx, SPI_InitStruct);
    }
}
...
void SPI_Master_Cmd(SPI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_Cmd(SPI_JDI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_Cmd(SPI_Slave_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
...
void SPI_Cmd(SPI_TypeDef *SPIx, FunctionalState NewState)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_Cmd((SPI_JDI_TypeDef *)SPIx, NewState);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_Cmd((SPI_Slave_TypeDef *)SPIx, NewState);
    }
    else
    {
        SPI_Master_Cmd(SPIx, NewState);
    }
}

void SPI_Master_SendBuffer(SPI_TypeDef *SPIx, uint8_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
    ...
    {
        /* Read TFNF bit status in SPI_SR register: SET is Tx FIFO is not full */
        while (!(SPIx->SPI_SR & SPI_FLAG_TFNF));
        SPIx->SPI_DR[0] = (*pBuf++);
    }
}

void SPI_Master_JDI_SendBuffer(SPI_JDI_TypeDef *SPIx, uint8_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_SendBuffer(SPI_Slave_TypeDef *SPIx, uint8_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
...
void SPI_SendBuffer(SPI_TypeDef *SPIx, uint8_t *pBuf, uint16_t len)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SendBuffer((SPI_JDI_TypeDef *)SPIx, pBuf, len);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_SendBuffer((SPI_Slave_TypeDef *)SPIx, pBuf, len);
    }
    else
    {
        SPI_Master_SendBuffer(SPIx, pBuf, len);
    }
}

void SPI_Master_SendHalfWord(SPI_TypeDef *SPIx, uint16_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_SendHalfWord(SPI_JDI_TypeDef *SPIx, uint16_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_SendHalfWord(SPI_Slave_TypeDef *SPIx, uint16_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
...
void SPI_SendHalfWord(SPI_TypeDef *SPIx, uint16_t *pBuf, uint16_t len)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SendHalfWord((SPI_JDI_TypeDef *)SPIx, pBuf, len);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_SendHalfWord((SPI_Slave_TypeDef *)SPIx, pBuf, len);
    }
    else
    {
        SPI_Master_SendHalfWord(SPIx, pBuf, len);
    }
}

void SPI_Master_SendWord(SPI_TypeDef *SPIx, uint32_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_SendWord(SPI_JDI_TypeDef *SPIx, uint32_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_SendWord(SPI_Slave_TypeDef *SPIx, uint32_t *pBuf, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
...
void SPI_SendWord(SPI_TypeDef *SPIx, uint32_t *pBuf, uint16_t len)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SendWord((SPI_JDI_TypeDef *)SPIx, pBuf, len);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_SendWord((SPI_Slave_TypeDef *)SPIx, pBuf, len);
    }
    else
    {
        SPI_Master_SendWord(SPIx, pBuf, len);
    }
}

void SPI_Master_ClearINTPendingBit(SPI_TypeDef *SPIx, uint16_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_ClearINTPendingBit(SPI_JDI_TypeDef *SPIx, uint16_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_ClearINTPendingBit(SPI_Slave_TypeDef *SPIx, uint16_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
    ...
    if (SPI_IT & SPI_INT_TXO)
    {
        (void)SPIx->SPI_SLV_TXOICR;
    }

    if (SPI_IT & SPI_INT_RXO)
    {
        (void)SPIx->SPI_SLV_RXOICR;
    }

    if (SPI_IT & SPI_INT_RXU)
    {
        (void)SPIx->SPI_SLV_RXUICR;
    }

    if (SPI_IT & SPI_INT_FAE)
    {
        (void)SPIx->SPI_SLV_FAEICR;
    }

    if (SPI_IT & SPI_INT_TUF)
    {
        (void)SPIx->SPI_SLV_TXUICR;
    }

    if (SPI_IT & SPI_INT_RIG)
    {
        (void)SPIx->SPI_SLV_SSRICR;
    }
}
...
void SPI_ClearINTPendingBit(SPI_TypeDef *SPIx, uint16_t SPI_IT)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_ClearINTPendingBit((SPI_JDI_TypeDef *)SPIx, SPI_IT);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_ClearINTPendingBit((SPI_Slave_TypeDef *)SPIx, SPI_IT);
    }
    else
    {
        SPI_Master_ClearINTPendingBit(SPIx, SPI_IT);
    }
}

void SPI_Master_SendData(SPI_TypeDef *SPIx, uint32_t Data)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_SendData(SPI_JDI_TypeDef *SPIx, uint32_t Data)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_SendData(SPI_Slave_TypeDef *SPIx, uint32_t Data)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));

    SPIx->SPI_SLV_DR[0] = Data;
}
...
void SPI_SendData(SPI_TypeDef *SPIx, uint32_t Data)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SendData((SPI_JDI_TypeDef *)SPIx, Data);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_SendData((SPI_Slave_TypeDef *)SPIx, Data);
    }
    else
    {
        SPI_Master_SendData(SPIx, Data);
    }
}

uint32_t SPI_Master_ReceiveData(SPI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
uint32_t SPI_Master_JDI_ReceiveData(SPI_JDI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
uint32_t SPI_Slave_ReceiveData(SPI_Slave_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));

    return (uint32_t)SPIx->SPI_SLV_DR[0];
}
...
uint32_t SPI_ReceiveData(SPI_TypeDef *SPIx)
{
    ...
    if (SPIx == SPI1)
    {
        return SPI_Master_JDI_ReceiveData((SPI_JDI_TypeDef *)SPIx);
    }
    else if (SPIx == SPI_SLAVE)
    {
        return SPI_Slave_ReceiveData((SPI_Slave_TypeDef *)SPIx);
    }
    else
    {
        return SPI_Master_ReceiveData(SPIx);
    }
}

uint8_t SPI_Master_GetTxFIFOLen(SPI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
uint8_t SPI_Master_JDI_GetTxFIFOLen(SPI_JDI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));

    SPI_JDI_TXFLR_TypeDef spi_0x20 = {.d32 = SPIx->SPI_TXFLR};
    return spi_0x20.b.TXTFL;
}

uint8_t SPI_Slave_GetTxFIFOLen(SPI_Slave_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));

    SPI_SLV_TXFLR_TypeDef spi_0x20 = {.d32 = SPIx->SPI_SLV_TXFLR};
    return spi_0x20.b.TXTFL;
}
...
uint8_t SPI_GetTxFIFOLen(SPI_TypeDef *SPIx)
{
    ...
    if (SPIx == SPI1)
    {
        return SPI_Master_JDI_GetTxFIFOLen((SPI_JDI_TypeDef *)SPIx);
    }
    else if (SPIx == SPI_SLAVE)
    {
        return SPI_Slave_GetTxFIFOLen((SPI_Slave_TypeDef *)SPIx);
    }
    else
    {
        return SPI_Master_GetTxFIFOLen(SPIx);
    }
}

uint8_t SPI_Master_GetRxFIFOLen(SPI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
uint8_t SPI_Master_JDI_GetRxFIFOLen(SPI_JDI_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));

    SPI_JDI_RXFLR_TypeDef spi_0x24 = {.d32 = SPIx->SPI_RXFLR};
    return spi_0x24.b.RXTFL;
}

uint8_t SPI_Slave_GetRxFIFOLen(SPI_Slave_TypeDef *SPIx)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));

    SPI_SLV_RXFLR_TypeDef spi_0x24 = {.d32 = SPIx->SPI_SLV_RXFLR};
    return spi_0x24.b.RXTFL;
}
...
uint8_t SPI_GetRxFIFOLen(SPI_TypeDef *SPIx)
{
    ...
    if (SPIx == SPI1)
    {
        return SPI_Master_JDI_GetRxFIFOLen((SPI_JDI_TypeDef *)SPIx);
    }
    else if (SPIx == SPI_SLAVE)
    {
        return SPI_Slave_GetRxFIFOLen((SPI_Slave_TypeDef *)SPIx);
    }
    else
    {
        return SPI_Master_GetRxFIFOLen(SPIx);
    }
}

void SPI_Master_ChangeDirection(SPI_TypeDef *SPIx, uint16_t dir)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_ChangeDirection(SPI_JDI_TypeDef *SPIx, uint16_t dir)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
/**
  * \brief  Change SPI direction mode by SPI master.
  * \param  SPIx: Select the SPI peripheral. \ref SPI_Declaration
...
void SPI_ChangeDirection(SPI_TypeDef *SPIx, uint16_t dir)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_ChangeDirection((SPI_JDI_TypeDef *)SPIx, dir);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_ChangeDirection(SPIx, dir);
    }
}

void SPI_Master_SetReadLen(SPI_TypeDef *SPIx, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_SetReadLen(SPI_JDI_TypeDef *SPIx, uint16_t len)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_SetReadLen(SPI_TypeDef *SPIx, uint16_t len)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SetReadLen((SPI_JDI_TypeDef *)SPIx, len);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_SetReadLen(SPIx, len);
    }
}

void SPI_Master_SetCSNumber(SPI_TypeDef *SPIx, uint8_t number)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));

    /* set cs number */
    SPIx->SPI_SER = BIT(number);
}

void SPI_Master_JDI_SetCSNumber(SPI_JDI_TypeDef *SPIx, uint8_t number)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_SetCSNumber(SPI_TypeDef *SPIx, uint8_t number)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SetCSNumber((SPI_JDI_TypeDef *)SPIx, number);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_SetCSNumber(SPIx, number);
    }
}

ITStatus SPI_Master_GetINTStatus(SPI_TypeDef *SPIx, uint32_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
    ...
    if ((SPIx->SPI_ISR & SPI_IT) != (uint32_t)RESET)
    {
        bit_status = SET;
    }
...
ITStatus SPI_Master_JDI_GetINTStatus(SPI_JDI_TypeDef *SPIx, uint32_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
ITStatus SPI_Slave_GetINTStatus(SPI_Slave_TypeDef *SPIx, uint32_t SPI_IT)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
    ...
    /* Check the status of the specified SPI flag */
    if ((SPIx->SPI_SLV_ISR & SPI_IT) != (uint32_t)RESET)
    {
        bit_status = SET;
    }
...
ITStatus SPI_GetINTStatus(SPI_TypeDef *SPIx, uint32_t SPI_IT)
{
    ...
    if (SPIx == SPI1)
    {
        return SPI_Master_JDI_GetINTStatus((SPI_JDI_TypeDef *)SPIx, SPI_IT);
    }
    else if (SPIx == SPI_SLAVE)
    {
        return SPI_Slave_GetINTStatus((SPI_Slave_TypeDef *)SPIx, SPI_IT);
    }
    else
    {
        return SPI_Master_GetINTStatus(SPIx, SPI_IT);
    }

}

void SPI_Master_INTConfig(SPI_TypeDef *SPIx, uint16_t SPI_IT, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_INTConfig(SPI_JDI_TypeDef *SPIx, uint16_t SPI_IT, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_INTConfig(SPI_Slave_TypeDef *SPIx, uint16_t SPI_IT, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
...
void SPI_INTConfig(SPI_TypeDef *SPIx, uint16_t SPI_IT, FunctionalState NewState)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_INTConfig((SPI_JDI_TypeDef *)SPIx, SPI_IT, NewState);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_INTConfig((SPI_Slave_TypeDef *)SPIx, SPI_IT, NewState);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_INTConfig(SPIx, SPI_IT, NewState);
    }
}

FlagStatus SPI_Master_GetFlagState(SPI_TypeDef *SPIx, uint16_t SPI_FLAG)
{
    /* Check the parameters */
    assert_param(IS_SPIM_PERIPH(SPIx));
...
#endif
    /* Check the status of the specified SPI flag */
    if ((SPIx->SPI_SR & SPI_FLAG) != (uint8_t)RESET)
    {
        /* SPI_FLAG is set */
        bitstatus = SET;
    }
...
FlagStatus SPI_Master_JDI_GetFlagState(SPI_JDI_TypeDef *SPIx, uint16_t SPI_FLAG)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
FlagStatus SPI_Slave_GetFlagState(SPI_Slave_TypeDef *SPIx, uint16_t SPI_FLAG)
{
    /* Check the parameters */
    assert_param(IS_SPIS_PERIPH(SPIx));
    ...
    /* Check the status of the specified SPI flag */
    if ((SPIx->SPI_SLV_SR & SPI_FLAG) != (uint8_t)RESET)
    {
        /* SPI_FLAG is set */
        bitstatus = SET;
    }
...
FlagStatus SPI_GetFlagState(SPI_TypeDef *SPIx, uint16_t SPI_FLAG)
{
    ...
    if (SPIx == SPI1)
    {
        return SPI_Master_JDI_GetFlagState((SPI_JDI_TypeDef *)SPIx, SPI_FLAG);
    }
    else if (SPIx == SPI_SLAVE)
    {
        return SPI_Slave_GetFlagState((SPI_Slave_TypeDef *)SPIx, SPI_FLAG);
    }
    else
    {
        return SPI_Master_GetFlagState(SPIx, SPI_FLAG);
    }
}

void SPI_Master_GDMACmd(SPI_TypeDef *SPIx, SPIGdmaTransferRequests_TypeDef SPI_GDMAReq,
...
void SPI_Master_JDI_GDMACmd(SPI_JDI_TypeDef *SPIx, SPIGdmaTransferRequests_TypeDef SPI_GDMAReq,
                            FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_Slave_GDMACmd(SPI_Slave_TypeDef *SPIx, SPIGdmaTransferRequests_TypeDef SPI_GDMAReq,
                       FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPI_ALL_PERIPH(SPIx));
    ...
    {
        /* Enable the selected SPI GDMA request */
        SPIx->SPI_SLV_DMACR |= SPI_GDMAReq;
    }
    else
    {
        /* Disable the selected SPI GDMA request */
        SPIx->SPI_SLV_DMACR &= (uint16_t)~(SPI_GDMAReq);
    }
}
                 ...
                 FunctionalState NewState)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_GDMACmd((SPI_JDI_TypeDef *)SPIx, SPI_GDMAReq, NewState);
    }
    else if (SPIx == SPI_SLAVE)
    {
        SPI_Slave_GDMACmd((SPI_Slave_TypeDef *)SPIx, SPI_GDMAReq, NewState);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_GDMACmd(SPIx, SPI_GDMAReq, NewState);
    }
}

void SPI_Master_ChangeClock(SPI_TypeDef *SPIx, uint32_t prescaler)
{
    assert_param(IS_SPIM_PERIPH(SPIx));
...
void SPI_Master_JDI_ChangeClock(SPI_JDI_TypeDef *SPIx, uint32_t prescaler)
{
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
...
void SPI_ChangeClock(SPI_TypeDef *SPIx, uint32_t prescaler)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_ChangeClock((SPI_JDI_TypeDef *)SPIx, prescaler);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_ChangeClock(SPIx, prescaler);
    }
}

void SPI_Master_SetRxSampleDly(SPI_TypeDef *SPIx, uint32_t delay)
{
    assert_param(IS_SPIM_PERIPH(SPIx));
    ...
    SPI_RSDR_TypeDef spi_0xf0 = {.d32 = SPIx->SPI_RSDR};
    spi_0xf0.b.RSD = delay & 0xFF;
    SPIx->SPI_RSDR = spi_0xf0.d32;
...
void SPI_Master_JDI_SetRxSampleDly(SPI_JDI_TypeDef *SPIx, uint32_t delay)
{
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
    ...
    /* Set SPI Rx sample delay */
    SPI_JDI_RSDR_TypeDef spi_0xf0 = {.d32 = SPIx->SPI_RSDR};
    spi_0xf0.b.RSD = delay & 0xFF;
    SPIx->SPI_RSDR = spi_0xf0.d32;
...
void SPI_SetRxSampleDly(SPI_TypeDef *SPIx, uint32_t delay)
{
    ...
    if (SPIx == SPI1)
    {
        SPI_Master_JDI_SetRxSampleDly((SPI_JDI_TypeDef *)SPIx, delay);
    }
    else if ((SPIx == SPI0) || (SPIx == SPI2))
    {
        SPI_Master_SetRxSampleDly(SPIx, delay);
    }
}
...
/**
  ...
  * \return None
  */
void SPI_WrapModeStartTx(SPI_JDI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * \return None
  */
void SPI_InverseCSActivePolarity(SPI_JDI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * \return None
  */
void SPI_DriveMOSILow(SPI_JDI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * \return  None
  */
void SPI_PullMOSIEn(SPI_JDI_TypeDef *SPIx, FunctionalState NewState)
{
    /* Check the parameters */
    assert_param(IS_SPIM_JDI_PERIPH(SPIx));
    assert_param(IS_FUNCTIONAL_STATE(NewState));
