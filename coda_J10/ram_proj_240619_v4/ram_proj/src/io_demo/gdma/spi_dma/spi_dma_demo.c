...
uint8_t sendbuf[TEST_SIZE];
uint8_t readbuf[TEST_SIZE];
uint8_t spi_tx_dma_ch_num = 0xa5;
uint8_t spi_rx_dma_ch_num = 0xa5;
...
void spi_tx_dma_handler(void);
void spi_rx_dma_handler(void);
...
void spi_send_dma_init(void)
{
    ...
    {
        APP_PRINT_TRACE0("tx channel request fail");
        return;
    }
...
/**
  ...
  * @return  void
  */
void spi_receive_dma_init(void)
{
    for (uint16_t i = 0; i < TEST_SIZE; i++)
    {
        readbuf[i] = 0;
    ...
    {
        APP_PRINT_TRACE0("rx channel request fail");
        return;
    }
...
void spi_tx_dma_handler(void)
{
    ...
    APP_PRINT_TRACE0("spi_tx_dma_handler");
}
...
void spi_rx_dma_handler(void)
{
    ...
    APP_PRINT_TRACE0("spi_rx_dma_handler");
}
