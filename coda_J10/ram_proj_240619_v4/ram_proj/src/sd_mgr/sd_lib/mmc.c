...
T_SD_STATUS MMC_ResetCard(void)
{
    ...
    {
        SDIO_PRINT_ERROR0("MMC_ResetCard: error ret = SD_CMD_INHIBIT_BEFORE_RESET ");
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        SDIO_PRINT_ERROR0("MMC_ResetCard: error ret = SD_SDIO_CMD_INHIBIT_AFTER_RESET ");
...
static T_SD_STATUS MMC_CheckResp1(uint32_t cmd)
{
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: error = SD_SDIO_CMD_INHIBIT_AFTER_RESET ");
        return SD_SDIO_CMD_INHIBIT_AFTER_RESET;
    }
    ...
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: cmd error = SD_CMD_ERROR ");
        return SD_CMD_ERROR;
    }
    ...
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_ADDR_OUT_OF_RANGE ");
        return SD_ADDR_OUT_OF_RANGE;
    }

    if (response & MMC_OCR_ADDR_MISALIGNED)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_ADDR_MISALIGNED ");
        return SD_ADDR_MISALIGNED;
    }

    if (response & MMC_OCR_BLOCK_LEN_ERR)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_BLOCK_LEN_ERR ");
        return SD_BLOCK_LEN_ERR;
    }

    if (response & MMC_OCR_ERASE_SEQ_ERR)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_ERASE_SEQ_ERR ");
        return SD_ERASE_SEQ_ERR;
    }

    if (response & MMC_OCR_BAD_ERASE_PARAM)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_BAD_ERASE_PARAM ");
        return SD_BAD_ERASE_PARAM;
    }

    if (response & MMC_OCR_WRITE_PROT_VIOLATION)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_WRITE_PROT_VIOLATION ");
        return SD_WRITE_PROT_VIOLATION;
    }

    if (response & MMC_OCR_LOCK_UNLOCK_FAILED)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_LOCK_UNLOCK_FAILED ");
        return SD_LOCK_UNLOCK_FAILED;
    }

    if (response & MMC_OCR_COM_CRC_FAILED)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_COM_CRC_FAILED ");
        return SD_COM_CRC_FAILED;
    }

    if (response & MMC_OCR_ILLEGAL_CMD)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_ILLEGAL_CMD ");
        return SD_ILLEGAL_CMD;
    }

    if (response & MMC_OCR_CARD_ECC_FAILED)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_CARD_ECC_FAILED ");
        return SD_CARD_ECC_FAILED;
    }

    if (response & MMC_OCR_CC_ERROR)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_CC_ERROR ");
        return SD_CC_ERROR;
    }

    if (response & MMC_OCR_GENERAL_UNKNOWN_ERROR)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_GENERAL_UNKNOWN_ERROR ");
        return SD_GENERAL_UNKNOWN_ERROR;
    }

    if (response & MMC_OCR_CID_CSD_OVERWRIETE)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_CID_CSD_OVERWRITE ");
        return SD_CID_CSD_OVERWRITE;
    }

    if (response & MMC_OCR_WP_ERASE_SKIP)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_WP_ERASE_SKIP ");
        return SD_WP_ERASE_SKIP;
    }

    if (response & MMC_OCR_CARD_ECC_DISABLED)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_CARD_ECC_DISABLED ");
        return SD_CARD_ECC_DISABLED;
    }

    if (response & MMC_OCR_ERASE_RESET)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_ERASE_RESET ");
        return SD_ERASE_RESET;
    }

    if (response & MMC_OCR_AKE_SEQ_ERROR)
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_AKE_SEQ_ERROR ");
        return SD_AKE_SEQ_ERROR;
    }
    /*  The card will expect ACMD */
    if (!(response & MMC_OCR_SD_APP_CMD))
    {
        SDIO_PRINT_ERROR0("MMC_CheckResp1: response error = SD_APP_CMD ");
        return SD_APP_CMD;
    }
...
T_SD_STATUS MMC_GetExtCSD(uint8_t buf[512])
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_GetExtCSD: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_GetExtCSD: err sd_status = 0x%x, line:%d", sd_status, __LINE__);
    }
...
T_SD_STATUS MMC_GetCardInfo(void *p_info)
{
    ...
    else
    {
        ...
        SDIO_PRINT_INFO1("eMMC sector count: %d", sec_count);
    ...
    SDIO_PRINT_INFO6("MMC_GetCardInfo: card_type: %d, DeviceSize: 0x%x, DeviceSizeMul: 0x%x, RdBlockLen: 0x%x, capacity: 0x%x, block_size: 0x%x",
...
T_SD_STATUS MMC_InitCard(void *p_dev)
{
    ...
    {
#if defined(IC_TYPE_BB2)
        p_ext_csd = os_mem_alloc(OS_MEM_TYPE_SHARING, 512);
#elif defined(IC_TYPE_BBPRO3)
        p_ext_csd = malloc(512);
#else
#error Undefined IC type
#endif
        if (!p_ext_csd)
        {
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_ResetCard error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_GetOCR error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_GetCID error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("SD_InitCard: MMC_SetRCA error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_GetCSD error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_SelectCmd error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_SetHighSpeedMode error, sd_status:%d", sd_status);
        return sd_status;
    }
    ...
    if (sd_cfg.sdh_bus_width == SD_BUS_WIDTH_4B)
    {
        ...
        if (sd_status != SD_OK)
        {
            DBG_DIRECT("MMC_InitCard: MMC_SetBusWide error, sd_status:%d", sd_status);
            return sd_status;
        }
    }
    ...
    if (sd_status != SD_OK)
    {
        DBG_DIRECT("MMC_InitCard: MMC_GetExtCSD error, sd_status:%d", sd_status);
        return sd_status;
    }

    p_dev_info->initialized = true;
    SDIO_PRINT_INFO0("MMC_InitCard: status: SD_OK");
    return sd_status;
}
...
T_SD_STATUS MMC_ReadSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_ReadSingleBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_ReadSingleBlock: err sd_status = 0x%x, line:%d", sd_status, __LINE__);
    }
                               ...
                               uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_ReadMultiBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
...
T_SD_STATUS MMC_WriteSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_WriteSingleBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_WriteSingleBlock: err sd_status = 0x%x, line:%d", sd_status, __LINE__);
        return sd_status;
    }
    ...
    {
        SDIO_PRINT_ERROR0("MMC_WriteSingleBlock: err rsp = sd_write_protection_violation");
        return SD_WRITE_PROT_VIOLATION;
    }
                                ...
                                uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_WriteMultiBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_WriteMultiBlock: err sd_status = 0x%x, line:%d", sd_status, __LINE__);
        return sd_status;
    }
    ...
    if (p_sd_host_prop->SDH_GetResponse(SDH_RSP0) & MMC_OCR_WRITE_PROT_VIOLATION)
    {
        SDIO_PRINT_ERROR0("MMC_WriteMultiBlock: err res = sd_write_protection_violation");
        return SD_WRITE_PROT_VIOLATION;
    }
...
T_SD_STATUS MMC_Earse(uint32_t start_addr, uint32_t end_addr)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_Earse: sd_status = 0x%x, line:%d", sd_status, __LINE__);
        return sd_status;
    }
    ...
    if (p_sd_host_prop->SDH_WaitCmdLineIdle() != SD_OK)
    {
        SDIO_PRINT_ERROR0("MMC_Earse: CMD line is not idle");
        return SD_SDIO_CMD_INHIBIT;
    }
...
T_SD_STATUS MMC_StopTransfer(void)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_StopTransfer: sd_status = 0x%x, line:%d", sd_status, __LINE__);
    }
    return sd_status;
}
...
T_SD_STATUS MMC_AsyncReadSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_AsyncReadSingleBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
                                    ...
                                    uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_AsyncReadMultiBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
...
T_SD_STATUS MMC_AsyncWriteSingleBlock(uint32_t sector, uint32_t buf, uint16_t block_size)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_AsyncWriteSingleBlock: SDH_DataConfig, eror status= 0x%x, line:%d",
                          sd_status, __LINE__);
        return sd_status;
    }
                                     ...
                                     uint16_t block_count)
{
    ...
    if (sd_status != SD_OK)
    {
        SDIO_PRINT_ERROR2("MMC_AsyncWriteMultiBlock: SDH_DataConfig, eror status= 0x%x, line:%d", sd_status,
                          __LINE__);
        return sd_status;
    }
