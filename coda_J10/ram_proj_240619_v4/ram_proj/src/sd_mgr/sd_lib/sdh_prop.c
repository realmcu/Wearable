...
extern T_SD_STATUS SDH_SetBusWide(uint32_t bus_wide);
...
__attribute__((aligned(4))) SDIO_ADMA2TypeDef  SDIO_ADMA2_DescTab[2];
#elif defined(IC_TYPE_BBPRO3)
__align(4) SDIO_ADMA2TypeDef  SDIO_ADMA2_DescTab[2];
#endif
...
static T_SD_STATUS SDH_WaitCmdLineIdle(void)
{
    ...
    while (SDIO_GetFlagStatus(SDIO_FLAG_CMD_INHIBIT) == SET)
    {
        ...
        if (time_out == 0)
        {
            //SDIO_PRINT_ERROR0("SDH_WaitCmdLineIdle: error, software timeout");
            return SD_SDIO_CMD_INHIBIT;
        }
    }
    SDH_ClockCmd(DISABLE);
    platform_delay_us(20000);
    return SD_OK;
}
...
static T_SD_STATUS SDH_WaitCmdDatComplete(void)
{
    ...
    while (SDIO_GetFlagStatus(SDIO_FLAG_CMD_DAT_INHIBIT) == SET)
    {
        ...
        if (time_out == 0)
        {
            //SDIO_PRINT_ERROR0("SDH_WaitCmdDatComplete: error, software timeout");
...
{
//#if defined(IC_TYPE_BB2)
//    SDIO_ADMA2_DescTab = os_mem_alloc(OS_MEM_TYPE_BUFFER, 2 * sizeof(SDIO_ADMA2TypeDef));
//#endif
